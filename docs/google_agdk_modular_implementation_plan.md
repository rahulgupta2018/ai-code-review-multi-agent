# Agentic Code Review on Google GADK – Modular Implementation Plan

> Successor to `docs/google_gadk_implementation_plan.md`. Incorporates modular delivery slices, operational readiness, and governance improvements while retaining every architectural commitment from the original plan.

---

## 1. Executive Summary
- **Objective**: Deliver a memory-first, multi-agent code review platform running entirely on Google GADK with backwards-compatible outputs and developer experience.
...
1. **Feature-flagged adoption**: `analysis.use_gadk` governs rollout; legacy executor remains callable until GA sign-off.
2. **Vertical slices before horizontal scale**: The first production milestone is a Code Analyzer MVP on GADK with baseline memory reads. Only then do we expand memory learning and additional agents.
...
4. **Observable by default**: Every runtime action surfaces telemetry in the GADK Dev Portal plus Stackdriver/Grafana dashboards.
...
- `src/integrations/gadk/runtime_factory.py`: Factory for GADK runtime sessions, feature flags, telemetry hooks.
- `src/core/orchestrator/smart_master_orchestrator.py`: Maintains logic to pick agents, now extended with GADK session management and Redis-backed coordination.
...
- `code_analyzer`, `engineering_practices`, `security_standards`, `carbon_efficiency`, `cloud_native`, `microservices`—all memory aware and running on GADK.
- Each agent includes:
...
- Augment with GADK toggles, Dev Portal endpoints, memory tuning, LLM fallback order, cost constraints, redis/sqlite connection details.
...
| --- | --- | --- | --- |
| **Runtime & Infrastructure** | GADK runtime, Dev Portal, containerization, IAM, networking | Platform Eng | Leads environment setup and runtime upgrades. |
...
- Purpose: Prove we can run Code Analyzer on GADK with baseline memory reads, deterministic tooling, and identical outputs to legacy path.
- Scope: Runtime factory, tool adapters for Code Analyzer heuristics, `analysis.use_gadk` flag, Dev Portal trace, `outputs/code_analyzer` parity check.
...
- **Tests**: Unit (tool contracts), integration (runtime session), regression (legacy vs GADK comparison), chaos scenarios.
...
**Goals**: GADK preview access, shared tooling image, Dev Portal container.

| Deliverable | Tasks | Tests & Evidence | Exit Criteria |
| --- | --- | --- | --- |
| Tooling Image | Build `gadk-tooling` image with GADK CLI, dev portal, Poetry, project scripts. Publish to registry. | CI build artifact, smoke-run `gadk --version`. | Image available, documentation updated. |
...
### Increment 1 – GADK Runtime MVP (Week 4-5)
- Implement `runtime_factory.py`, register placeholder tools, feature flag wiring.
- CLI command `scripts/run_code_analysis.py --use-gadk` to launch GADK session with stub tool returning static response.
- Dev Portal trace verifying session lifecycle.

### Increment 2 – Code Analyzer MVP Parity (Week 5-7)
- Lift complexity, pattern, architecture, LLM insight, quality control tools into GADK modules with typed schemas.
- `tool_adapters.py` wires dependencies (memory manager in read-only mode).
- Contract tests comparing GADK tool outputs vs legacy functions.
- Run orchestrator in GADK mode for Code Analyzer, capture diff vs legacy outputs (target <1% variance; mismatches reviewed).
...
- Update `SmartMasterOrchestrator` to drive GADK sessions with Redis state, WebSocket broadcasting, and fallback to legacy path.
...
- Monitoring + alerting (Stackdriver metrics, Grafana dashboards, GADK telemetry exports).
...
| **Integration (Agent)** | GADK session with agent-specific tools, memory interactions | pytest + GADK runtime harness |
...
4. **Runtime Deployment**: Helm charts for GADK runtime, Dev Portal, API layer, WebSocket service, memory storage sidecars.
...
- **Delta Sync**: During dual-run, capture writes from both legacy and GADK paths, reconcile via change-data-capture job.
...
- **Runbooks**: For GADK runtime restart, Dev Portal failure, Redis degradation, LLM provider outage, memory corruption, API errors.
...
| Tech Lead (Runtime) | GADK integration, infra decisions, runtime quality gates | Platform TL |
...
- **Enablement Plan**: Workshops on GADK tooling, Dev Portal navigation, new CLI/API usage.
...
- **Adoption Metrics**: Track % analyses on GADK, mean time to onboard engineer, satisfaction surveys.
...
| --- | --- | --- | --- | --- |
| GADK API changes | Runtime outages | Medium | Pin version, adapter layer, monitor release notes | Platform TL |
...
- **MVP Parity**: Code Analyzer runs on GADK, outputs parity within tolerance, feature flag available.
...
### A. Directory Map
- `src/integrations/gadk/`: runtime factory, tool adapters, credentials helpers.
...
> With this modular roadmap, each team can land value in smaller, testable chunks, maintain quality, and keep the multi-agent GADK migration on schedule while covering every original commitment.
